...
def index():
    ...
    # user sends buy/sell request
    if request.method == "POST":
        action = request.form.get("makeAct")
        print(action)
        if not action:
            return apology("Invalid action")

        [act, symb] = action.split(" ")
...
def buy():
    ...
    if request.method == "POST":
        ...
        if (len(currStockPort) == 0):
            db.execute("INSERT INTO owned_stock (user_id, symbol, buy_tot, num_shares, sold_tot, sold_shares) VALUES(?, ?, ?, ?, ?, ?)",
                       session["user_id"], symbol, purchase_price, shares, 0, 0)

        # case own/owned stock
        else:
            newShares = currStockPort[0]["num_shares"]+shares
            newTot = currStockPort[0]["buy_tot"] + purchase_price
            db.execute("UPDATE owned_stock SET buy_tot = ?, num_shares = ? WHERE user_id = ? AND symbol = ?",
                       newTot, newShares, session["user_id"], symbol)
    ...
    if "symb" in session:
        symb = session["symb"]
        del session["symb"]
        return render_template("buy.html", symb=symb)

    return render_template("buy.html", symb="-1")
...
def sell():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO purchase_history (symbol, price, num_shares, purchase_time) VALUES(?, ?, ?, DATETIME(\"now\"));",
                   sellSymb, -currPrice, sellShare)
        ...
        db.execute("UPDATE owned_stock SET sold_shares = ?, buy_tot = ?, sold_tot = ?, num_shares = ? WHERE user_id = ? AND symbol = ?;",
                   newShareSold, buyTot, sellTot, newShareOwned, session["user_id"], sellSymb)
    ...
    if "symb" in session:
        symb = session["symb"]
        del session["symb"]
        return render_template("sell.html", owned=owned, symb=symb)
    return render_template("sell.html", owned=owned, symb="-1")
